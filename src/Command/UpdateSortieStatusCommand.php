...
class UpdateSortieStatusCommand extends Command
{
     ...
     * @return int
     */
    public function statusEnCours(SymfonyStyle $io): int
    {
    ...
    public function statusCloture(SymfonyStyle $io): int
    {
        // 1. On cherche les sorties à mettre à jour
        $sortiesToUpdate = $this->sortieRepository->createQueryBuilder('s')
            ->where('s.users.length < :nbMax')
            ->andWhere('s.etat = :ouvert')
            ->setParameter('nbMax', 's.nbRegistration')
            ->setParameter('ouvert', Etat::OU->value)
            ->getQuery()
            ->getResult();
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->statusCloture($io);
        return $this->statusEnCours($io);
